...
单列表提供一条灰色关注分界线。更新时间严格晚于截止点的 Session 在线上，早于或等于截止点的在线下；有新活动时自动移到线上，相同时间的 Session 不拆开。长按拖柄450毫秒后可拖到行间，靠近边缘会滚动列表，只在列表内松手时保存。短按、提前移动、取消、Escape、窗口失焦，以及拖动期间目录时间变化均不提交。方向键、Home 和 End 也可移动分界点。时间点保存在本浏览器的视图存储中，旧版缺失该字段时默认全部需关注；分界点位于未加载的历史中时保持原值并提示继续加载。分组模式明确提供切到单列表关注视图的入口，搜索不显示分界线。分界线不会删除、隐藏、归档或中断任何会话。
