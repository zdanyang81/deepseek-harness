...
单列表提供一条灰色关注分界线。更新时间晚于截止点的 Session 在线上，更早的在线下；同一时间使用稳定的 Session ID 边界，支持在每两个相邻行间准确放线。有新活动时自动移到线上。长按拖柄450毫秒后可拖到行间，靠近边缘会滚动列表，只在列表内松手时保存。短按、提前移动、取消、Escape、窗口失焦，以及拖动期间目录时间变化均不提交。方向键、Home 和 End 也可移动分界点。时间点及可选的同时间 ID 边界保存在本浏览器的视图存储中，首次使用保存当前时间，旧条目默认均在线下；已有数字时间点保留原含义；分界点位于未加载的历史中时保持原值并提示继续加载。分组模式提供明确的**切到时间列表使用关注线**入口，搜索不显示分界线。分界线不会删除、隐藏、归档或中断任何会话。
