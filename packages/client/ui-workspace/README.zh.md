...
首次使用的浏览器默认进入带关注分界线的单一时间列表；已有的按工作区分组偏好保持不变。**最近更新**通过全局运行时钩子取得 Session，严格按 `updatedAt` 从新到旧排列，同一时间用 Session id 稳定排序。**手动排序**使用浏览器本地单列表账号里真实保存的顺序，并支持原生 HTML 行拖拽；新出现或未见过的 Session 追加在末尾，后续活动不会把一行顶上去。Workspace 拖拽顺序仍由 Host 持久化。分组模式在各 Workspace 内按时间排序，并记住分组关闭或显示五条 Session 的状态，提供临时的**展开其余**控件。从 Workspace 行创建 Session 时会先打开分组。单列表省略空的左侧状态槽，但保留可见的 Session 状态。

两种排序下，单列表都为灰色关注分界线预留32px独立行，拖柄和线条不覆盖相邻会话标题或抢占行点击区域。28×20 拖柄右缘与 Session 时间列共用 8px 右侧内边距；分界线不再渲染可见的「关注/可忽略」文案，该含义仍由拖柄的 `aria-label` 与 `title` 提供。最近更新模式下，更新时间晚于截止点的 Session 在线上，更早的在线下；同一时间使用稳定的 Session ID 边界，有新活动时自动移到线上。手动排序模式下，同一条分界线是独立持久化的行计数，不会跟着被拖动的 Session 走；删掉线上的一行会把计数减一，尾部分页不改变计数。切换模式时两条边界各自恢复。缺失的手动计数会按当时最近更新的可视位置初始化一次。两种模式都保留 `data-attention-cutoff` 和 `data-attention-index`，让现网侧栏 overlay 能定位灰线；手动模式下 cutoff 写非时间标记 `manual`。按下拖柄即可拖到行间；拖动时会话行轨道不动，由跟随指针的 ghost 和插入标记指示目标位置，靠近边缘会滚动列表，只在列表内松手时保存。仅尾部追加旧页且已见行的ID、时间和位置逐项不变时，保留手势并允许继续拖到新加载行。短按且未移动、取消、Escape、窗口失焦，以及已见前缀的时间修改、重排、删除或插入均不提交。方向键、Home 和 End 也可移动分界线。时间截止点和手动行计数都保存在本浏览器的视图存储中。最近更新首次使用保存当前时间，旧条目默认均在线下；已有数字时间点保留原含义；该截止点位于未加载的历史中时保持原值，并仅通过拖柄 `title` 说明需要继续加载。分组模式提供明确的**切到时间列表使用关注线**入口，搜索不显示分界线。分界线不会删除、隐藏、归档或中断任何会话。
...
- **组合后的侧栏不提供首问预览**：侧栏使用 Session 标题；当前运行时 API 和列表元数据均不提供首问加载器或字段。行级可选显示辅助函数不代表完整组合已具备该能力。
